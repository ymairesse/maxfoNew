...
<p>Ce {$jour} {$date} à {$heure}, quelqu'un (sans doute vous) à l'adresse IP {$identiteReseau.ip} ({$identiteReseau.hostname}) a demandé le changement de mot passe pour l'utilisateur
    <strong>{$identite.acronyme}</strong>.</p>
...
<a href="{$BASEDIR}index.php?action=renewPasswd&amp;acronyme={$identite.acronyme}&amp;token={$link}">{$BASEDIR}index.php?action=renewPasswd&amp;acronyme={$identite.acronyme}&amp;token={$link}</a>
